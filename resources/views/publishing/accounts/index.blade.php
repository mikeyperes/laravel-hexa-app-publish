...
<div class="space-y-4">
    ...
    <div class="flex flex-wrap items-center gap-3">
        ...
        <a href="{{ route('settings.users.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700 inline-flex items-center gap-1">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-5 py-3">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('publish.accounts.show', $user->id) }}" class="text-gray-400 hover:text-blue-600" title="Publishing Profile">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </a>
                                <a href="{{ route('settings.users.show', $user->id) }}" class="text-gray-400 hover:text-yellow-600" title="Edit User">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </a>
                                @if($user->id !== auth()->id())
                                <form method="POST" action="{{ route('settings.users.destroy', $user->id) }}" onsubmit="return confirm('Delete user {{ $user->name }}? This cannot be undone.')" class="inline">
                                    @csrf @method('DELETE')
