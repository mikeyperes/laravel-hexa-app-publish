@php
    ...
    $resolvedValues = app(\hexa_core\Forms\Services\FormHydrationService::class)->hydrate(
        ...
        $formValues ?? [],
        [],
        ['context' => $mode, 'mode' => $mode, 'record' => $template ?? null]
    );
    $fields = collect($form->fieldsForContext($mode));
    $groupedFields = $fields->groupBy(fn ($field) => $field->metaValue('section', 'additional'));
    $sectionOrder = ['account', 'basic', 'content', 'copy', 'defaults', 'additional'];
    $orderedSections = array_values(array_unique(array_merge($sectionOrder, $groupedFields->keys()->all())));
    $sectionClasses = [
        'account' => 'grid gap-4 grid-cols-1 md:grid-cols-3 pb-4 border-b border-gray-200',
        'basic' => 'grid gap-4 grid-cols-1 md:grid-cols-3',
        'content' => 'grid gap-4 grid-cols-1 md:grid-cols-2',
        'copy' => 'grid gap-4 grid-cols-1 md:grid-cols-2',
        'defaults' => 'grid gap-4 grid-cols-1 md:grid-cols-2',
        'additional' => 'grid gap-4 grid-cols-1 md:grid-cols-2 pt-4 border-t border-gray-200',
    ];
    $sectionTitles = [
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-5">
        <form x-ref="form" action="{{ $form->actionUrl() ?? '#' }}" method="POST" class="space-y-5">
            ...
            @foreach($orderedSections as $section)
                ...
                <div>
                    @if(isset($sectionTitles[$section]) || !in_array($section, ['account', 'basic', 'content', 'copy', 'defaults'], true))
                        <div class="mb-3">
                            <h3 class="text-sm font-semibold text-gray-700">{{ $sectionTitles[$section] ?? \Illuminate\Support\Str::headline($section) }}</h3>
                        </div>
                    @endif
        ...
        <div class="flex items-center gap-3">
            <button type="button" @click="save('active')" :disabled="saving" class="bg-blue-600 text-white px-5 py-2 rounded-lg text-sm hover:bg-blue-700 disabled:opacity-60 inline-flex items-center gap-2">
                ...
                <span x-text="saving && saveType === 'active' ? '{{ $mode === 'edit' ? 'Saving...' : 'Creating...' }}' : '{{ $mode === 'edit' ? 'Save Changes' : 'Create Template' }}'"></span>
            </button>
            <button type="button" @click="save('draft')" :disabled="saving" class="bg-gray-200 text-gray-700 px-5 py-2 rounded-lg text-sm hover:bg-gray-300 disabled:opacity-60 inline-flex items-center gap-2">
                ...
                <span x-text="saving && saveType === 'draft' ? '{{ $mode === 'edit' ? 'Saving Draft...' : 'Saving Draft...' }}' : 'Save as Draft'"></span>
            </button>
...
@once
    @push('scripts')
    <script>
    function articlePresetFormPage(config) {
        return {
            companyModels: config.companyModels || {},
            selectedCompany: config.selectedCompany || '',
            formAiEngine: config.aiEngine || '',
            saving: false,
            saveType: '',
            resultMessage: '',
            resultSuccess: false,
