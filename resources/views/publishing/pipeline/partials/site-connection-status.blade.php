...
<div x-show="selectedSite" x-cloak class="mt-2">
    <div class="flex items-center gap-2 rounded-lg px-3 py-2" :class="siteConn.status === true ? 'bg-green-50' : (siteConn.status === false ? 'bg-red-50' : 'bg-gray-50')">
        ...
        <span class="text-xs" :class="siteConn.status === true ? 'text-green-700' : (siteConn.status === false ? 'text-red-700' : 'text-gray-500')" x-text="siteConn.testing ? (siteConn.status === true ? 'Connected — loading authors...' : 'Verifying WordPress connection...') : (siteConn.status === true ? (siteConn.message || 'Connected') : (siteConn.status === false ? siteConn.message : (siteConn.message || 'Loading connection state...')))"></span>
