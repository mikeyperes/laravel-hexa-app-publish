            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 cursor-pointer {{ $log->success ? '' : 'bg-red-50' }}" @click="expandedRow = expandedRow === {{ $log->id }} ? null : {{ $log->id }}">
                    <td class="px-4 py-3 whitespace-nowrap">
                        <p class="text-gray-800">{{ $log->created_at->format('M j, Y') }}</p>
                        <p class="text-xs text-gray-400">{{ $log->created_at->format('g:ia') }} <span class="text-gray-300">UTC {{ $log->created_at->format('H:i') }}</span></p>
                    </td>
                    <td class="px-4 py-3">
                ...
                @empty
                <tr><td colspan="9" class="px-4 py-8 text-center text-gray-400">No AI activity logged yet.</td></tr>
                @endforelse
            </tbody>
        </table>
