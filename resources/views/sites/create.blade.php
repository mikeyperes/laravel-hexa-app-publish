...
<div class="max-w-2xl" x-data="siteForm()">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-5">
        ...
        <div>
            ...
            <input type="text" x-model="form.name" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="e.g. Grit Daily">
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">URL <span class="text-red-500">*</span></label>
            <input type="url" x-model="form.url" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="https://gritdaily.com">
        </div>
        ...
        <template x-if="form.connection_type === 'wp_rest_api'">
            <div class="space-y-4 border-l-4 border-blue-200 pl-4">
                <p class="text-xs text-gray-500">
                    <strong>How to set up:</strong> In WordPress admin, go to Users > Profile > Application Passwords.
                    Create a new application password and paste it below.
                </p>
                <div>
                ...
                <div>
                    ...
                    <input type="password" x-model="form.wp_application_password" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="xxxx xxxx xxxx xxxx xxxx xxxx">
                </div>
            </div>
        </template>
...
function siteForm() {
    return {
        ...
        async save() {
            ...
            try {
                const res = await fetch('{{ route("publish.sites.store") }}', {
                    method: 'POST',
                ...
                this.resultMessage = data.message || (data.success ? 'Site added.' : 'Failed.');
